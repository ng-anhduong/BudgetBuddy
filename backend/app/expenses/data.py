...
@bp.route('/export_csv_file', methods=['POST'])
@jwt_required()
def export_csv_file():
    # Default currency of the app
    currency = "SGD"
    ...
    # 1. Write CSV to memory
    output = io.StringIO()
    writer = csv.writer(output)

    # 2. Write header (extract column names from the model)
    writer.writerow(["category", "amount", "currency", "description", "date"])  # customize to match your Expense model

    # 3. Write rows
    for row in rows:
        writer.writerow([
            row.category.value, # type: ignore
            round(amt(row.amount, row.currency.value), 2), # type: ignore
